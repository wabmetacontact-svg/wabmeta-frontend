...
const WalletManagement: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Content */}
      <div className="bg-[#0a0e27] rounded-2xl border border-white/[0.1] shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-[#050816] border-b border-white/[0.1]">
                <tr>
                  <th className="px-6 py-4 font-medium text-gray-400">Organization</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {requests.length === 0 ? (
                  ...
                  requests.map((req) => (
                    <tr key={req.id} className="hover:bg-[#050816] transition-colors">
                      <td className="px-6 py-4">
                        <p className="font-semibold text-white">{req.organization?.name || 'Unknown'}</p>
                        <p className="text-xs text-gray-500">{req.organization?.planType || 'FREE'}</p>
                      </td>
                      <td className="px-6 py-4 text-gray-400">{req.user?.email || 'Unknown'}</td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-gray-400 max-w-xs truncate" title={req.reason}>
                          {req.reason}
                        </p>
                      </td>
                      <td className="px-6 py-4">
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-[#050816] border-b border-white/[0.1]">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium text-gray-400">Credit Limit</th>
                  <th className="px-6 py-4 font-medium text-gray-400">Status</th>
                  <th className="px-6 py-4 font-medium text-gray-400 text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {wallets.length === 0 ? (
                  ...
                  wallets.map((w) => (
                    <tr key={w.id} className="hover:bg-[#050816] transition-colors">
                      <td className="px-6 py-4">
                        <p className="font-semibold text-white">{w.organization?.name || 'Unknown'}</p>
                        <p className="text-xs text-gray-400">{w.organization?.planType || 'FREE'}</p>
                      </td>
                      <td className="px-6 py-4">
                        <span className="font-bold text-white">{formatINR(w.balance * 100)}</span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2 flex-wrap">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            w.isActive ? 'bg-green-100 text-green-700' : 'bg-[#0a0e27]/[0.04] text-gray-500'
                          }`}>
      ...
      {showAdjustModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-[#0a0e27] rounded-2xl max-w-md w-full shadow-xl">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowAdjustModal(false)} className="p-1.5 hover:bg-[#0a0e27]/[0.04] rounded-lg transition-colors">
            ...
            <div className="p-6 space-y-4">
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 py-2 rounded-xl border ${
                      adjustType === 'admin_credit' 
                        ...
                        : 'border-white/[0.1] text-gray-400 hover:bg-[#050816]'
                    }`}
                  >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-white/[0.12] rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
            ...
            <div className="p-6 border-t border-gray-100 flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 text-gray-400 hover:bg-[#0a0e27]/[0.04] rounded-xl font-medium"
              >
                Cancel
              </button>
              <button
          ...
          <div className="bg-[#0a0e27] rounded-2xl max-w-md w-full shadow-xl">
            <div className={`p-6 border-b border-gray-100 flex items-center justify-between ${
              ...
              <button
                ...
                className="p-1.5 hover:bg-[#0a0e27]/60 rounded-lg transition-colors"
              >
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 border border-white/[0.12] rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm"
                  placeholder={
                    toggleTarget.currentlyActive

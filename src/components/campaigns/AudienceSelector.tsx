// src/components/campaigns/AudienceSelector.tsx - FIXED (Light + Dark mode)
    ...
    <div className="space-y-6">
      ...
      {audienceType === 'group' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 space-y-4 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-gray-900 dark:text-white">Select Group</h4>
          ...
          <select
            ...
            className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-all"
          >
      ...
      {audienceType === 'tags' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-gray-900 dark:text-white">Select Tags</h4>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                  selectedTags.includes(tag)
                    ...
                    : 'bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10'
                }`}
              >
          ...
          {availableTags.length === 0 && (
            <p className="text-center text-gray-500 dark:text-gray-400 py-4">No tags available</p>
          )}
        </div>
      )}
      ...
      {audienceType === 'manual' && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-gray-900 dark:text-white">Select Contacts</h4>
            <button
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-gray-900 dark:text-white placeholder-gray-400 transition-all"
            />
          </div>
              ...
              <label
                ...
                className={`flex items-center space-x-3 p-3 rounded-lg cursor-pointer transition-all ${
                  selectedContacts.includes(contact.id)
                    ...
                    : 'bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 hover:bg-emerald-50/50 dark:hover:bg-emerald-500/5 hover:border-emerald-300 dark:hover:border-emerald-500/30'
                }`}
              >
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 text-emerald-500 border-gray-300 dark:border-gray-600 rounded focus:ring-emerald-500"
                />
                <div className="flex-1 min-w-0">
                ...
                {contact.tags.length > 0 && (
                  <div className="flex space-x-1">
                    {contact.tags.slice(0, 2).map((tag) => (
                      <span key={tag} className="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded text-xs">
                        {tag}
                      </span>
                    ))}
                  </div>
      ...
      {/* CSV Selection */}
      {audienceType === 'csv' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 space-y-4 border-2 border-dashed border-gray-300 dark:border-gray-600 text-center">
          <input
            type="file"
            accept=".csv"
            id="csv-upload"
          ...
          {csvContacts && csvContacts.length > 0 ? (
            <div className="flex flex-col items-center gap-3">
              <div className="h-12 w-12 rounded-full bg-emerald-100 dark:bg-emerald-500/20 flex items-center justify-center mb-1">
                <FileText className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
              </div>
              <div>
                <h4 className="font-semibold text-gray-900 dark:text-white">{csvFileName || 'Uploaded CSV'}</h4>
              ...
              <button
                ...
                className="mt-2 flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 rounded-lg transition-colors border border-red-200 dark:text-red-400 bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/30"
              >
                <X className="w-4 h-4" />
                Remove CSV
              </button>
            ...
            <>
              <div className="h-12 w-12 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center mx-auto">
                <FileText className="w-6 h-6 text-gray-500 dark:text-gray-400" />
              </div>
              <h4 className="font-semibold text-gray-900 dark:text-white">Upload Contacts CSV</h4>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Ensure your CSV has a column named "Phone" or "Number".
              </p>
              <label
                htmlFor="csv-upload"
                className="inline-block mt-2 px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg cursor-pointer font-medium transition-all shadow-sm"
              >
                Choose CSV File
              </label>
            </>
      ...
      <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-xl p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-gray-900 dark:text-white">Total Recipients</span>
          </div>
